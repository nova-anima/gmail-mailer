...
- 受信メール一覧 / 送信済みメール一覧（件名と本文の冒頭を表示。**各スレッドの最新メッセージ時刻の新しい順**に整列）
- 一覧クリックで **本文全文ビュー**（すぐ返信入力にはせず、まず全文を表示）→「返信」ボタンで返信ビューへ、の3段画面構成
- スレッド内は **最新メッセージが上**（新しい順）。**最新 5 件のみ表示**し、6 件以上は「さらに読み込む」で 5 件ずつ古いメッセージを追加表示
- 本文表示は HTML メールはそのまま、プレーンテキストは改行を保持（サンドボックス iframe でメール内スクリプトを無効化）
- **全員に返信**（既定の宛先は「全員に返信」のアドレス。**To / Cc / Bcc を編集可能**。引用を本文末尾に自動付与し、同一スレッドへ送信）
- 新規作成（To / Cc / Bcc 指定）
- 送信は団体アカウントとして行われ、団体アカウントの「送信済み」に残る
- PC / スマホ対応のレスポンシブレイアウト
...
- **一覧取得**: Gmail 高度なサービス（`Gmail.Users.Threads.list/get` のメタデータ）でスレッドの件名・差出人・スニペット・時刻を軽量取得。
- **本文取得**: `GmailApp`（`getThread` → `getBody()` / `getPlainBody()`）を使用。Base64 / 文字コードの復号を GAS に任せることで、文字化けやデコード例外を回避（プレーンテキストは改行を保持して表示）。スレッド内は最新 5 件ずつ本文を取得。
- **送信 / 返信**: `Gmail.Users.Messages.send`（`raw` ＋ `threadId`）で団体アカウントとして送信し、同一スレッドに連結（`In-Reply-To` / `References` も付与）。Bcc にも対応。
- **スコープ**: `https://mail.google.com/`（GmailApp・高度なサービス両方をカバー）、`script.external_request`（トークン交換）、`groups`（`@googlegroups.com` 判定）。
...
```bash
git clone https://github.com/nova-anima/gmail-mailer.git
cd gmail-mailer
npm install
```
...
- **スレッド表示**: 最新メッセージが上（新しい順）。初期表示は最新 5 件で、6 件以上は「さらに読み込む」で 5 件ずつ古いメッセージを取得します（必要分のみ本文取得）。
...
│   ├── Mail.gs           Gmail 一覧（高度なサービス）/ 本文（GmailApp）/ 返信・送信
